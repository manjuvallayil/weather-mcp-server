...
This is the "broker" layer (analogous to alpaca_broker.py in the reference project).
MCP tool functions should call these helpers and return the results; no raw HTTP
calls belong in the MCP server file.
...
def geocode(location: str) -> dict:
    """Resolve a location string to lat/lon using Open-Meteo Geocoding API.

    Args:
        location: City name, e.g. "Chicago", "Auckland, New Zealand"

    Returns:
        dict with keys: name, country, latitude, longitude, timezone
        Raises ValueError if location cannot be resolved.
    """
    resp = httpx.get(_GEOCODE_URL, params={"name": location, "count": 1, "language": "en"})
...
def get_current(latitude: float, longitude: float) -> dict:
    ...
    Returns:
        dict with: temperature_c, feels_like_c, humidity_pct, wind_speed_kmh,
                   wind_direction_deg, conditions (text), weather_code
    """
    params = {
        "latitude": latitude,
        "longitude": longitude,
    ...
    resp = httpx.get(_FORECAST_URL, params=params)
    resp.raise_for_status()
...
def get_daily_forecast(latitude: float, longitude: float, days: int = 7) -> list[dict]:
    ...
    Returns:
        List of dicts, one per day, each with: date, temp_high_c, temp_low_c,
